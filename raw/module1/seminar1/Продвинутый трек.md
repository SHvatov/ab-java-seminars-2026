...
`Java Basics` `Переменные и типы` `Условия и циклы` `Методы и классы` `Git (ветки, MR)` `Установленный JDK 21+` `Установленный IntelliJ IDEA` `Установленный VisualVM` `Maven (базовый уровень)`
...
Презентация. Цель — не пересказать учебник, а показать **зачем** и **как принципы работают вместе**. Аудитория продвинутая, поэтому темп выше, определения короче, акцент на взаимосвязи.

**Ключевые тезисы:**

- **Инкапсуляция** — не просто геттеры и сеттеры. Это сокрытие деталей реализации и защита инвариантов. Пример из Си: `open()` в `fcntl.h` — один контракт, разные реализации под каждую ОС. Клиентский код не знает и не должен знать, что происходит под капотом.
- **Наследование** — механизм установления IS-A и переиспользования кода. Имеет цену: жёсткая связанность, каскадные изменения в глубоких иерархиях. Краткий намёк на ISP — подробнее на семинаре 2.
- **Полиморфизм** — два вида сегодня: **подтипов** (runtime, через override) и **перегрузка** (compile-time). Шаблонный (generics) — позже.
- **Взаимосвязь:** полиморфизм подтипов невозможен без наследования; наследование без полиморфизма теряет смысл; инкапсуляция — фундамент, без которого всё течёт наружу.
...
> **Обсуждение:** оба варианта нарушают LSP. Метод не принадлежит базовому классу — убираем его из `Instrument`, оставляем только в `Stock`.
