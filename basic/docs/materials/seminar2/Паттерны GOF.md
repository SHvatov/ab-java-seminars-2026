...
6. Паттерны, похожие по структуре, но разные по назначению
...
```java
void example(String channel, String recipient, String message) {
...
Каждый новый канал уведомлений — правка этого метода; логика выбора конкретного класса размазана по вызывающему коду
и дублируется в каждом месте, где нужно создать уведомление.
...
Реестр в примере выше — статический и заполняется вручную. Альтернатива — механизм автоматического обнаружения
реализаций (service discovery): каждая фабрика регистрируется декларативно, а не явной записью в общий реестр,
который поэтому вообще не нужно вести и изменять. В Java для этого есть штатный механизм —
`java.util.ServiceLoader`: реализация перечисляется в файле `META-INF/services/<имя интерфейса>`, а вызывающий код
получает уже готовый список реализаций без обращения к какому-либо реестру напрямую.
...
Часть паттернов из разных разделов реализуется практически одинаковым набором классов — интерфейс и один
оборачивающий его класс-обёртка. Различие между ними не в структуре кода, а в том, **зачем** эта обёртка нужна:

- **Decorator и Proxy** — структурно неотличимы (объект, реализующий тот же интерфейс, что и оборачиваемый, и
  делегирующий ему вызовы). Decorator добавляет новое поведение поверх существующего; Proxy контролирует доступ к
  объекту, не меняя его поведение по сути (отложенная инициализация, проверка прав, логирование обращений).
- **Adapter и Facade** — оба «прячут» интерфейс за другим интерфейсом. Adapter подгоняет интерфейс одного класса
  под уже существующий, ожидаемый клиентом; Facade вводит новый, более простой интерфейс поверх нескольких
  классов, у которых своего общего интерфейса раньше не было вовсе.
- **Strategy и Command** — оба инкапсулируют «единицу поведения» в отдельный объект с одним методом. Strategy —
  про выбор между взаимозаменяемыми алгоритмами решения одной и той же задачи; Command — про сам факт превращения
  запроса в объект, который можно поставить в очередь, отложить или отменить, независимо от того, один это
  алгоритм или несколько.
