...
```java
Notification create(String channel, String recipient, String message) {
...
Логика выбора конкретной реализации размазана по вызывающему коду: там, где нужно создать уведомление, придётся
скопировать (или вызвать) весь этот блок целиком, и каждый новый канал означает правку уже существующего метода.
...
Сам реестр в этом примере — обычная статическая карта, которую нужно вручную пополнять при каждой новой фабрике.
Более автоматизированный вариант — механизм обнаружения реализаций (service discovery), при котором реестр вообще
не заполняется вручную: каждая реализация лишь заявляет о себе декларативно. В Java эту роль играет
`java.util.ServiceLoader` — реализации перечисляются в файле `META-INF/services/<полное имя интерфейса>`, а
получить готовый список найденных реализаций можно одним вызовом `ServiceLoader.load(...)`, не трогая никакой общий
реестр.
...
Несколько пар паттернов из разных разделов на уровне кода выглядят почти одинаково — интерфейс плюс класс,
делегирующий вызовы другому объекту, реализующему тот же интерфейс. Разница между ними — не в наборе классов, а в
цели, ради которой эта обёртка вводится:

- **Proxy и Decorator** — одна и та же форма (обёртка с тем же интерфейсом, что у оборачиваемого объекта). Proxy
  контролирует доступ к объекту — откладывает инициализацию, проверяет права, логирует обращения, — не расширяя
  его поведение по существу. Decorator, напротив, именно добавляет новое поведение поверх уже существующего.
- **Facade и Adapter** — оба скрывают один интерфейс за другим. Facade вводит новый, упрощённый интерфейс там, где
  общего интерфейса у нескольких классов раньше не было. Adapter ничего не упрощает — он подгоняет существующий
  интерфейс класса под другой интерфейс, который уже ожидает клиентский код.
- **Command и Strategy** — оба сводятся к «объекту, содержащему один метод». Strategy инкапсулирует выбор между
  взаимозаменяемыми способами решить одну и ту же задачу. Command — про сам запрос, превращённый в объект: его
  можно поставить в очередь, отложить, отменить или залогировать, вне зависимости от того, есть ли у задачи вообще
