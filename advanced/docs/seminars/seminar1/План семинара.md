...
Удаляем `InstrumentFormatter`, переносим логику в подклассы:
...
**Композиция для контраста** — `Portfolio` создаёт и владеет `Position`:
...
Пишем простую программу, которая создаёт оба массива и держит их в памяти:
...
**Подключение библиотеки через Maven.** Показываем, как добавить зависимость в `pom.xml`:
