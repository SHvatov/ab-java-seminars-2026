...
Показываем README проекта, пробегаемся по функциональным требованиям. Студенты клонируют репозиторий и создают ветку —
...
Через 8 минут — короткое обсуждение. Студенты называют сущности, семинарист фиксирует на доске и приходит к референсной
модели.
...
Удаляем `InstrumentFormatter`, переносим логику в подклассы. Итоговый код:
...
> **Обсуждение:** оба варианта нарушают LSP. Метод не принадлежит базовому классу — убираем его из `Instrument`,
> оставляем только в `Stock`. Именно так и сделано в итоговом
> [`Stock.java`](../../../src/main/java/academy/backend/market_pulse/model/Stock.java).
...
> **Акцент:** это хороший момент объяснить, что такое Maven-координаты (`groupId:artifactId:version`), где их искать (
> Maven Central), и как IntelliJ IDEA автоматически скачивает зависимости после изменения `pom.xml`.
